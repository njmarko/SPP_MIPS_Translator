...
public:
	...
	* @param v reference to the class that inherits the Visitor class
	*/
...
public:
	...
	* @param data reference to the SymbolTable class that will be the host for this visitor
	*/
...
public:
	...
	* @param v reference to the class that inherits the visitor abstract class
	*/
	...
	* @throw runtime_error (Semantic) if the lable that is being jump to doesn't exist 
	*/
	...
	* @param params to a list of strings that constans parameter variables
	...
	* @throw runtime_exception if the mem variable is already defined
	* @throw runtime_exception if the name format is incorrect
	*/
	...
	* @param name of the variable
	* @param type of the variable
	* @throw runtime_exception if the reg variable is already defined
	* @throw runtime_exception if the name format is incorrect
	*/
	...
	* @param lab_name label name
	* @throw runtime_exception if the variable or function with the same name already exists
	* @throw runtime_exception if the name format is incorrect
	*/
	...
	* @param v pointer to a variable that is being spilled into memory
	*/
	void addSpilledVar(Variable * v);

	/**
	* Creates the error message for semantic instruction errors
	* @param argPos position of the parameter that is insvalid (can be 0,1,2)
	* @param varType type of the variable that is invalid
	* @param i_type instruction type needed for printing the instruction format
	* @param faultyParam name of the invalid variable
	...
	* @param instrs const reference to a List of instruction pointers
	*/
